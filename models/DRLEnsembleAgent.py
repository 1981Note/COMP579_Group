# Reference: https://github.com/AI4Finance-Foundation/FinRL/blob/master/finrl/agents/stablebaselines3/models.py
...
from utils import config_ensemble_strategy
from env_stock_trading.env_stocktrading import StockTradingEnv
from preprocessor.preprocessors import data_split

MODELS = {"a2c": A2C, "ddpg": DDPG, "td3": TD3, "sac": SAC, "ppo": PPO}

MODEL_KWARGS = {x: config_ensemble_strategy.__dict__[f"{x.upper()}_PARAMS"] for x in MODELS.keys()}
...
class DRLEnsembleAgent:
    ...
    def get_model(
        ...
        return MODELS[model_name](
            policy=policy,
            env=env,
            tensorboard_log=f"{config_ensemble_strategy.TENSORBOARD_LOG_DIR_ENSEMBLE}/{model_name}",
            verbose=verbose,
    ...
    def train_model(model, model_name, tb_log_name, iter_num, total_timesteps=5000):
        ...
        model.save(
            f"{config_ensemble_strategy.TRAINED_MODEL_DIR_ENSEMBLE}/{model_name.upper()}_{total_timesteps // 1000}k_{iter_num}"
        )
        return model

    @staticmethod
    def get_validation_sharpe(iteration, results_dir, model_name):
    ...
    def _train_window(
        ...
        sharpe = self.get_validation_sharpe(i, self.results_dir, model_name=model_name)
